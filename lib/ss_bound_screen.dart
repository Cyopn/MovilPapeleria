...
void main() => runApp(const encuadernado());

class encuadernado extends StatelessWidget {
  const encuadernado({super.key});
...
class _PrintServiceScreenState extends State<PrintServiceScreen> {
  ...
  int _quantity = 1;
  int _impresionType = 0;
  int _selectedUploadIndex = -1;

  String _paperSize = 'Carta';
  String _pastaType = 'Pasta dura';
  String _boundType = 'Espiral plastico';
  String _coverColor = 'Rojo';

  DateTime _deliveryDate = _nextValidDeliveryDate(DateTime.now());
  ...
  static DateTime _nextValidDeliveryDate(DateTime from) {
    var date = _onlyDate(from);
    while (
  ...
  Future<void> _openPaymentModal() async {
    ...
    await PaymentModal.open(
      ...
      serviceContext: {
        ...
        'id_files': fileIds,
        'printType': _impresionType == 1 ? 'color' : 'bw',
        ...
        'boundType': _boundType == 'Encolada' ? 'encolada' : 'espiral',
        'coverType': _pastaType == 'Pasta dura' ? 'dura' : 'blanda',
  ...
  Future<void> _calculatePriceApi({List<Map<String, dynamic>>? uploads}) async {
    ...
    try {
      ...
      final payload = {
        ...
        'colorModes': _impresionType == 1 ? 'color' : 'bw',
        ...
        'type': 'bound',
        'coverType': _pastaType == 'Pasta dura' ? 'dura' : 'blanda',
        'bindingType': _boundType == 'Encolada' ? 'encolada' : 'espiral',
      };
  ...
  Future<void> _pickDeliveryDate() async {
    ...
    final initial = _deliveryDate.isBefore(today)
        ? _nextValidDeliveryDate(today)
        : _deliveryDate;
    ...
    setState(() {
      _deliveryDate = _onlyDate(selected);
    });
  }
  ...
  Widget _buildPriceBlocks() {
    ...
    final coverCost = parseNum(perSet['coverCost']);
    final bindingCost = parseNum(perSet['bindingCost']);
    final inkCost = parseNum(perSet['inkCost']);
    final paperCost = parseNum(perSet['paperCost']);
    final coverTotal = parseNum(total['coverCost']);
    final bindingTotal = parseNum(total['bindingCost']);
      ...
      children: [
        Container(
          ...
          child: Column(
            crossAxisAlignment: CrossAxisAlignment.start,
            children: [
              const Text('Precios de encuadernado',
                  style: TextStyle(fontWeight: FontWeight.bold)),
              const SizedBox(height: 8),
              if (_priceData == null)
                const Text('Sube un archivo para calcular precio.')
              else ...[
                Text('Cubierta: \$ ${coverCost.toStringAsFixed(2)}'),
                Text('Encuadernado: \$ ${bindingCost.toStringAsFixed(2)}'),
                Text(
                  'Precio por juego: \$ ${(coverCost + bindingCost).toStringAsFixed(2)}',
                  style: const TextStyle(fontWeight: FontWeight.w600),
                ),
                Text(
                  'Total encuadernado: \$ ${(coverTotal + bindingTotal).toStringAsFixed(2)}',
                ),
              ],
            ],
          ),
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      endDrawer: Drawer(
        child: ListView(
          children: const [
            DrawerHeader(child: Text('Opciones de Servicio')),
            ListTile(
                leading: Icon(Icons.settings), title: Text('Configuración')),
          ...
          SingleChildScrollView(
            child: Column(
              children: [
                PrintServiceHeader(
                  title: 'Encuadernado',
                  backgroundImage: 'assets/encuadernado.png',
                ...
                Padding(
                  ...
                  child: Column(
                    ...
                    children: [
                      _buildUploadsList(),
                      if (_uploads.isNotEmpty) const SizedBox(height: 20),
                      ...
                      Container(
                        ...
                        child: RadioListTile<int>(
                          ...
                          value: 0,
                          groupValue: _impresionType,
                          onChanged: (val) {
                            if (val == null) return;
                            setState(() => _impresionType = val);
                      ...
                      const SizedBox(height: 20),
                      _buildSectionTitle('Tipo de pasta'),
                      Container(
                        padding: const EdgeInsets.symmetric(horizontal: 15),
                        decoration: BoxDecoration(
                          color: Colors.grey.shade200,
                      ...
                      const SizedBox(height: 20),
                      _buildSectionTitle('Tipo de encuadernado'),
                      Container(
                        padding: const EdgeInsets.symmetric(horizontal: 15),
                        decoration: BoxDecoration(
                          color: Colors.grey.shade200,
                        ...
                        child: DropdownButtonHideUnderline(
                          child: DropdownButton<String>(
                            value: _boundType,
                            isExpanded: true,
                            items: ['Espiral plastico', 'Encolada']
                                .map(
                                  (e) => DropdownMenuItem(
                                    value: e,
                                    child: Text(e),
                      ...
                      const SizedBox(height: 20),
                      _buildSectionTitle('Color de cubierta'),
                      Container(
                        padding: const EdgeInsets.symmetric(horizontal: 15),
                        decoration: BoxDecoration(
                          color: Colors.grey.shade200,
                        ...
                        child: DropdownButtonHideUnderline(
                          child: DropdownButton<String>(
                            value: _coverColor,
                            isExpanded: true,
                            items: ['Rojo', 'Verde', 'Azul', 'Amarillo']
                                .map(
                                  (e) => DropdownMenuItem(
                                    value: e,
                                    child: Text(e),
                      ...
                      const Text(
                        'No se permiten fechas anteriores ni fines de semana.',

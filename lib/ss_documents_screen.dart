...
void main() => runApp(const documento());
...
class documento extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return MaterialApp(
      ...
      home: const SpecialDocumentScreen(),
    );
  }
}

class SpecialDocumentScreen extends StatefulWidget {
  const SpecialDocumentScreen({super.key});

  @override
  State<SpecialDocumentScreen> createState() => _SpecialDocumentScreenState();
}

class _SpecialDocumentScreenState extends State<SpecialDocumentScreen> {
  ...
  Future<void> _calculatePriceApi({List<Map<String, dynamic>>? uploads}) async {
    ...
    try {
      ...
      final payload = {
        ...
        'sets': sets,
        'type': 'docs',
        'documentType': _documentTypeApiValue(),
      };
  ...
  Widget _buildPriceCard() {
    ...
    final docsCost = parseNum(perSet['docsCost']);
    final docsCostTotal = parseNum(total['docsCost']);
      ...
      children: [
        Container(
          ...
          child: Column(
            crossAxisAlignment: CrossAxisAlignment.start,
            children: [
              const Text('Precios de documento especial',
                  style: TextStyle(fontWeight: FontWeight.bold)),
              const SizedBox(height: 8),
              if (_priceData == null)
                const Text('Sube un archivo para calcular precio.')
              else ...[
                Text('$_docType: \$ ${docsCost.toStringAsFixed(2)}'),
                Text('Precio por juego: \$ ${docsCost.toStringAsFixed(2)}'),
                Text('Total documento: \$ ${docsCostTotal.toStringAsFixed(2)}'),
              ],
            ],
          ),
        ),
            ...
            child: Column(
              children: [
                PrintServiceHeader(
                  ...
                  backgroundImage: 'assets/docespecial.png',
                      ...
                      Container(
                        ...
                        child: DropdownButtonHideUnderline(
                          child: DropdownButton<String>(
                            value: _docType,
                            isExpanded: true,
                            items: ['Tesis', 'Examen', 'Reporte', 'Otro']
                                .map(
                                  (e) => DropdownMenuItem(
                                    value: e,
                                    child: Text(e),
